...
def test_fio_rw(file_path):
    fio_installed = check_fio()
    if fio_installed:
        rw_args = ['write', 'read', 'rw', 'randwrite', 'randread', 'randrw']
        results = {}
        ...
        if checkdisk_s !=0:
            ...
            for rw in rw_args:
                (stat, out) = commands.getstatusoutput('fio -ioengine=libaio -bs=%s -direct=1\
                                                        ...
                                                        -filename=/dev/vdb\
                                                        -name="FIO with different rw mode"\
...
def _results():
    ...
    file_path = '/tmp/' + 'fio_rw_' + random_str()
    os.mknod(file_path)
    results_dict = test_fio_rw(file_path)
    output_info = {}
